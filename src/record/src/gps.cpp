...
void cal_dis(const std_msgs::String::ConstPtr& msg)
{
 ...
 record::lonlat msg3;
 std::stringstream ss;
 msg3.lon = cur_gps.lon;
 msg3.lat = cur_gps.lat;
 ...
 msg2.lon = goal_gps.lon;
 msg2.lat = goal_gps.lat;
 pub2.publish(msg2);
 ...
 ss << kw << "在当前位置的"<<ew<<sn<<"方向,距离当前位置"<<dis<<"米";
    }
 msg1.data = ss.str();
 nav.publish(msg1); 
 pub.publish(msg3);
...
int main(int argc, char **argv)
{
  ...
  ros::NodeHandle n;
  ros::Rate r(10);
  pub = n.advertise<record::lonlat>("sendgps", 1000); 
  nav = n.advertise<std_msgs::String>("speechnav", 1000);
  pub2 = n.advertise<record::lonlat>("goalposition", 1000);
  ros::Subscriber sub1 = n.subscribe("chatter", 1000, cal_dis); 
  r.sleep();
  ros::spin();
  return 0;
}
